...
for i in range(int(100*granularity)):
    ...
    try:
        ...
        pass
    # saving metrics
    train_loss, test_loss = np.zeros((int(100*granularity), runs, epochs)), np.zeros((int(100*granularity), runs,epochs))
    train_acc, test_acc = np.zeros((int(100*granularity), runs, epochs)), np.zeros((int(100*granularity), runs, epochs))
    train_time = np.zeros((int(100*granularity), runs, epochs))
    grd_time, sim_time, pred_time = np.zeros((int(100*granularity), runs, epochs)), np.zeros((int(100*granularity), runs, epochs)), np.zeros((int(100*granularity), runs, epochs))
    not_selected = np.zeros((int(100*granularity), runs, epochs))
    ...
    for run in range(runs):
        ...
        for epoch in range(0, epochs):
            ...
            for index in range(num_batches):
                ...
                train_time[i][run][epoch] += time.time() - start
            ...
            if subset:
                ...
                grd_time[i, run, epoch], sim_time[i, run, epoch], pred_time[i, run, epoch] = ordering_time, similarity_time, pre_time
                times_selected[run][indices] += 1
                not_selected[i, run, epoch] = np.sum(times_selected[run] == 0) / len(times_selected[run]) * 100
            else:
                pred_time = 0
            ...
            test_loss[i][run][epoch], test_acc[i][run][epoch] = score[0], score[1]
            train_loss[i][run][epoch], train_acc[i][run][epoch] = score_loss[0], score_loss[1]
            best_acc = max(test_acc[i][run][epoch], best_acc)

            grd = 'random_wor' if random else 'grd_normw'
        print(f'run: {run}, {grd}, subset_size: {subset_size}, epoch: {epoch}, test_acc: {test_acc[i][run][epoch]}, '
              f'loss: {train_loss[i][run][epoch]}, best_prec1_gb: {best_acc}, not selected %:{not_selected[i][run][epoch]}')
