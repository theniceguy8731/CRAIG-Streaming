...
def get_orders_and_weights(B, X, metric, smtk, no=0, stoch_greedy=0, y=None, weights=None, equal_num=False, outdir='.'):
    ...
    # Ensure all elements in order_mg_all have the same shape
    max_length = max(len(order) for order in order_mg_all)
    order_mg_all = [np.pad(order, (0, max_length - len(order)), 'constant') for order in order_mg_all]
    order_mg_all = np.array(order_mg_all)

    # Ensure all elements in cluster_sizes_all have the same shape
    max_length = max(len(cluster) for cluster in cluster_sizes_all)
    cluster_sizes_all = [np.pad(cluster, (0, max_length - len(cluster)), 'constant') for cluster in cluster_sizes_all]
